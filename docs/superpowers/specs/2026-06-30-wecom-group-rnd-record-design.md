...
**推论**：① 只能拿到"@机器人的纯文本/图片"；② 群↔需求映射**不能**靠群名/自动化喂字段自动完成，**必须人工一次性绑定**。
...
### 3.2 绑定指令
- 文本匹配 `#绑定 <key>`（容忍前后@文本/空格）→ 在「配色&样品需求单」按 `requirement_key`（样品编号等）查 record_id → 写 `group_record_map(chatid→record_id)` → 用 `response_url` 回"✅ 已关联 <需求>"。
- 未匹配到需求行 → 回"未找到编号 <key> 对应的需求，请核对"。
- 机器人在**未绑定群第一次被@** → 回引导"本群尚未关联需求，请发 `#绑定 <样品编号>`"。
...
- `#绑定` 解析 + 按 requirement_key 查 record_id + 写 map。
...
- 每个需求群：把机器人拉进群 + 发一次 `@项目开发管理助手 #绑定 <编号>`。
