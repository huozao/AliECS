...
| P4 调度内核进入 shadow | 2026-08-13 | [#308](https://github.com/huozao/AliECS/pull/308)、热修 [#309](https://github.com/huozao/AliECS/pull/309) | doc/T+ 共用字节一致的纯决策内核，生产保留 legacy 实际控制并把 candidate 决策写入真实 scheduled run。整分支终审与热修审查均无 open Critical/Important；根 unittest 最终 880 项全绿（6 个环境 skip），T+ 175 项全绿。PR #308 CI run [31702395185](https://github.com/huozao/AliECS/actions/runs/31702395185) 在 Python 3.12/PostgreSQL 16 上真实执行 scheduler integration：4 tests OK、无 skip；热修 CI run [31705540396](https://github.com/huozao/AliECS/actions/runs/31705540396) validate 与 migration-dry-run 均 success。infra `65536e3` 以 SOPS 将两个 txecs profile 的 doc/T+ mode 固定为 `shadow`。最终发布 run [31705766017](https://github.com/huozao/AliECS/actions/runs/31705766017) 的 `stage-business-cn-peer` success（`deploy-business-cn` skipped 正常），运行 source commit `a278fafda4f7712678880549d25f38c334e449ca`。生产 22/22 doc candidate schedule 与 legacy `15:30/86400` 一致，T+ candidate 与 legacy `01:00/86400` 一致；22 个 doc + 1 个 T+ 真实 scheduled run 已有 shadow 样本，全部 `mode=shadow`、`decision_match=true`、`due_delta_seconds=0`。P4 首次发布后新增 run 0、容器 active 计数 0、scheduler error/traceback 0、open alert 0，公网 `/health/` 与 `/sync/` 均 200。只开始 shadow，未切 active，P5 未开始。 |
...
- #308 首次上线后发现 22 个 doc job 的 candidate schedule 仍为空：P0 建空表早于 P1 建 job，而既有 `integration_sync_config` 没有再次 upsert，`PostgresStore.seed_platform_schedule()` 也尚未接入 worker。#309 将现有幂等 seeder 接到 doc preflight，缺 candidate 时以当前 legacy 配置初始化并同轮重读；未直接手改数据库伪造证据。
- 最终运行证据：doc worker 镜像 `t-4967772643ad`、启动时间 `2026-08-13T13:42:19Z`；T+ worker 镜像 `t-9106a273bf40`、验证性重启时间 `2026-08-13T13:11:33Z`；两者 restart count 均为 0，容器内 mode 均为 `shadow`。
- 首批 23 个样本尚未到真实 legacy 等待终点，因此 `observed_sleep_seconds` 仍未写入；这是持续 shadow 证据项，禁止补造。后续切 active 前仍须核对实际 sleep、manual/event、不一致率、due 偏差与错误率。
- 单 worker 回滚：只把对应 SOPS host input（`DOC_SYNC_SCHEDULER_MODE` 或 `TPLUS_SYNC_SCHEDULER_MODE`）改为 `legacy`，以 production gate 重渲染并 force-recreate 对应 worker；另一 worker 保持 `shadow`。当前不得把任一值改为 `active`。
