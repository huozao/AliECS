...
| BOM builder 搜父件/子件/原材料，规格型号一栏永远空白，用规格也搜不到 | 最新 `inventory_*.xlsx` 的 `Specification` 列是不是整列空 | **T+ 只回 `SelectFields` 点名的字段；没点名的仍出现在返回结构里、值为 null**，扁平化导出后就是「列在、整列空」，看起来像 T+ 里没填。2026-09-07 实测：导出 619 行 Specification 非空 0 条，而直接调 `inventory/Query` 带上 `Specification` 时前 1000 条里 250 条有值。白名单在 `services/tplus-sync-worker/src/tplus_datahub/modules/inventory/sync_stock.py`；后端搜索与前端下拉早就支持规格，缺的只是数据。**改完必须跑一轮全量同步才进 Excel**，改代码不改变已导出的文件 |
| BOM builder 搜索框逐字输入很卡 | 单次 `/v1/tplus/inventories` 的耗时 | 该接口原先每个请求都重新解析一次存货档案 Excel（2026-09-07 实测 0.58~0.73s/次，`iterrows` 只占 0.024s），编码查重走的还是同一个接口。已按 (路径, mtime, 大小) 缓存解析结果，档案每天换新文件名会自然失效 |
...
| 定时全量**每天同一时刻**失败、`error_json` 里 bom/inventory 都是 `status=999 EXERROR0001 网络异常`，白天手动跑却全通 | 失败时刻的分布：`SELECT to_char(started_at AT TIME ZONE 'Asia/Shanghai','MM-DD HH24:MI'), status FROM integration_sync_runs WHERE module='all' ORDER BY id DESC LIMIT 20;` | **T+ 服务端那侧在该时段不可用**，不是我们的链路。`EXERROR0001` 是畅捷通云网关转发不到 T+ 服务器时返回的。`inventory`（扁平档案，正常 9s）也一起挂就能排除慢查询超时。2026-08-06~08-10 连续 5 天 02:01 全挂、08-01~08-03 连续 3 天 23:30 全挂，而 12:22 / 18:38 / 07:3x 全部成功；08-10 09:08 白天只读探测三接口全 200。**处置是把执行时刻挪到 T+ 可用的时段**，不是改代码。凌晨不可用的具体原因与窗口边界至今未取证。2026-08-30 同族复发一次（详见下一行 10107） |
| 同步中心 T+ ERP 页 19 个资产**各挂一条**「需要处理的表」，看起来像 19 个故障 | `SELECT seq,name,status,message FROM sync_job_steps WHERE run_id=<该次 run>;` | **是一次运行里的一条级联，不是 19 个独立故障**——19 个资产共用 `chanjet.full` 一个作业，页面按资产逐行展示同一条告警。2026-08-30 01:02 CST 实测形态：步骤 1 `bom` 耗时 30s 读超时，其后 18 个步骤在 6 秒内全部返回 `{"code":"10107","messages":"T+ Server is not found."}`（10107 与上一行的 `EXERROR0001` 同源，都是畅捷通云网关转发不到 T+ 服务器）。判别「是不是我们这侧的问题」用白天只读探测，不落库：`docker exec business-cn-tplus-sync-worker-1 sh -c 'cd /app && PYTHONPATH=/app:/app/src python -c "…ChanjetClient().post(\"/tplus/api/v2/inventory/QueryPage\", {\"param\":{\"PageIndex\":1,\"PageSize\":1}})"'`——2026-08-30 08:56 CST 实测 inventory 2.31s / bom 0.66s 全 200，openToken 有效。前 6 天（08-23~08-28）连续成功、08-22 同一时刻出过同样的 10107，可据此排除「本次部署引入」 |
...
## 改执行时刻（anchor_time）：两处一起改，且改完必须重启才可靠生效（2026-08-30）
...
⚠️ **改完不要假设会自动生效——worker 正睡在一个进睡前一次性算好的长睡眠里。**
`worker_loop` 主循环每轮开头才 `_resolve_sync_config()`，睡眠按
`TPLUS_SYNC_POLL_SECONDS=30` 分片但 `remaining` 进睡即算定。

代码里**确实有热唤醒**（PR#290）：`_sleep_with_legacy_wake` 给分片循环传
`should_wake=_schedule_target_moved_earlier`，每片重读 `integration_sync_config`，
算出的新目标比进睡时的 `planned_due` 早 30 秒以上就 break，并打印
`T+ schedule target moved earlier, replanning sleep`。**但 2026-08-30 实测它没有触发**：
两处配置都改好后等了约 7 分钟毫无反应，`docker logs … | grep -c 'moved earlier'` 为 0，
日志停在上一轮的 `T+ sync worker sleeping: seconds=86190`。按判据推算本应触发
（`planned_due` = 08-30 17:00:59 UTC，新锚点 07:30 算出的目标已在过去），
**没触发的原因至今未定位**。所以：

- 结论只能是「热唤醒不可依赖」，不是「没有热唤醒」。⚠️ 该点 2026-08-30 首次记录时
  被我写成「从不重读 `anchor_time`」，是错的——机制存在，只是没生效。
- 已验证可靠的手段是重启：`docker restart business-cn-tplus-sync-worker-1`
  （重启前先确认 `integration_sync_requests` 没有 `pending`/`running`，
  两个全量并行会互相把对方本批未出现的记录标成 `missing_since`）。
