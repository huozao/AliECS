...
`0059`、`0060` 现通过 `-- migration: atomic` 显式启用新路径，SQL 自带 BEGIN/COMMIT 和表锁；
...
推进相同双时钟判据，每组只持久化最终当前观察；重跑从已有当前观察初始化，在线触发器判据不变。
同一恢复副本优化后0059整体78.39秒，0058为1.51秒、0060为0.08秒；这不是生产锁时长承诺。
...
回读：`schema_migrations` 中确认 `0058`、`0059`、`0060` 三条完整版本名；确认新增 5 张表、3 个触发器。
