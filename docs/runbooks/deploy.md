...
### V6 观察台迁移（0058 → 0059 → 0060 → 0062）

`0058` 保存用户已读状态；`0059` 新增历史/当前观察投影；`0060` 新增事件水位与缺序投影；
`0062` 将在线观察投影改为每个包内键只写一次，保留同一双时钟判据和不可变观察历史。
后两项首次回填需要分别锁住原始快照表、事件表的写入，读查询仍可执行。
原始快照和事件不删除。先核验备份可恢复、两张原始表的行数/大小、当前写入事务和允许的迁移窗口，
再迁移；本地合成测试耗时不能当生产锁时长。全部迁移成功后才发布依赖这些表的新后端。

⚠️ 「普通迁移执行器会把整个 SQL 文件放在单个事务中」自 2026-09-08 起确认不成立：
原执行器调用 psql 时逐语句提交，外部 timeout 终止 Docker 客户端也不能证明容器内 psql 已退出。
`0059`、`0060`、`0062` 现通过 `-- migration: atomic` 显式启用新路径，SQL 自带 BEGIN/COMMIT 和表锁；
执行器使用数据库端语句超时，psql 遇错停止并断开连接，未提交事务回滚，失败不自动重试。
既有未标记迁移的执行语义保持原样。

标记迁移复用 `PSQL_TIMEOUT_SECONDS`（默认 30，正整数）：它是每条数据库语句及连接的等待上限，
不是整个文件耗时上限；锁等待上限取该值与 5 秒中的较小值。迁移文件含多条语句，总耗时可超过该值。
正式窗口根据历史规模调整现有参数，不在 SQL 内固定生产回填时长。
2026-09-09 首次正式数据恢复演练：16650条原始快照展开824036条观察，旧版逐行更新当前投影在
120秒和900秒语句上限均失败；两次均回读原始16650条、两张派生表不存在、其他迁移连接0，未触碰正式库。
0059尚未应用到正式库，因此在该待发布迁移内局部改为按运行/字段/合约分组，在内存按原序号/元素次序
推进相同双时钟判据，每组只持久化最终当前观察；0062继续将这一组内存重放用于在线触发器，
每个包/键只持久化一次，重跑从已有当前观察初始化，判据不变。
同一恢复副本优化后0059整体78.39秒，0058为1.51秒、0060为0.08秒；0062为函数替换，
不承诺生产写入耗时，这些本地数字都不是生产锁时长承诺。
50,000行本地回归旧版因5秒语句超时失败，新版通过；另400行/8个当前键对照在线触发器，
覆盖空时钟、交叉时钟、双字段和重复合约元素，以及2行乱序重跑不重置当前状态。
正式首次应用使用命令级数据库语句上限120秒、锁等待5秒，回读完整投影后登记；后续部署按登记跳过。
若连接异常或部署被中断，先查数据库活动事务、迁移登记和投影完整性；确认已退出后再运行迁移，
不能凭外部进程结束立即重开一个执行器。若事务已提交而登记写入失败，投影保留，幂等重跑后补登记。

回读：`schema_migrations` 中确认 `0058`、`0059`、`0060`、`0062` 四条完整版本名；确认新增 5 张表、3 个触发器，
并用一个含重复合约元素的包确认 `market_review_current_observations` 每个键只保留最终序号。
按运行编号比较原始快照中有合约名的 quotes/bands 元素数与历史观察投影行数；
用原始事件序号的相邻差计算缺序区间，与事件缺序表逐区间核对；不要只看最高序号。
迁移前后原始证据不得减少。再核对最新视图、历史分页、历史/新增未读及用户隔离。
回退应用时保留追加式证据与投影，不通过删表清理已执行迁移。

本地验证使用 `V6_TEST_DATABASE_URL` 的随机隔离命名空间；直接 psql 回归另指定本地
`V6_TEST_POSTGRES_CONTAINER`。入口：`tests/test_market_review_storage.py`、
`tests/test_market_review_migration_runner.py`；不得将生产库传给测试。
