...
AliECS 需要把企业微信智能表格同步成 Postgres 中可查询的数据，供后台接口读取。第一版目标是完整同步，不做最近记录截断，不只同步表单入口，也不只同步字段。
...
- `doc-sync-worker`：读取环境变量，调用企业微信 API，完整同步字段、sheet、records，写入 Postgres 和同步运行日志。
- `backend-api`：不直接调用企业微信 API，只提供同步结果、同步状态、同步日志查询接口，并创建后台手动同步请求。
