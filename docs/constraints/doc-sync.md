...
| 档 | 对象 | SLA |
|---|---|---|
| 控制面 / 配置表 | 管理面板 3、系统配置 5、飞书 ChatGPT 会话管理台 8、`tplus.parent_match`、`wecom.locator_mirror` | 24h ⚠️ 见下节，这一档等于调度周期，每晚必误报 |
...
## SLA 不得等于调度周期——余量为零的阈值每天都会误报（2026-09-01 定案）

上一节「控制面 24h」那一档配的 86400 正好等于 doc_sync 的调度周期 86400。
两者同刻度时**余量为零**：只要今天比昨天晚一分钟跑完，就会在「昨天完成 +24h」到
「今天完成」之间开出一个 stale 窗口。实测两晚，每晚 16 条告警 + 16 条恢复：

| 日期 | 开出 | 解除 | 条数 |
|---|---|---|---|
| 08-31 | 00:32:34 / 00:33:36 | 01:13:33 | 16 开 + 16 解 |
| 09-01 | 00:46:39 / 00:47:11 | 01:22:25 | 16 开 + 17 解（多的一条是 `wecom.doc.2` 的 failed 恢复）|

`wecom.doc.19511` 的完成时间：08-30 00:32:33 → 08-31 00:46:14 → 09-01 00:55:02。
08-31 完成于 00:46:14，09-01 00:46:14 越线，25 秒后那次轮询开告警；本轮 00:55:02
成功即解除。**页面上那一轮全是 success，误报不掩盖任何真实故障。**

起跑时间为什么在往后漂：`anchor_time` 配的是 00:30 且 `_next_due()` 每天按 00:30
重算（不是相对上次累加），漂的是**唤醒**——睡眠循环旧实现按名义 30 秒扣减
`remaining`，而每步实际耗时 = 30 秒 sleep + 这一 poll 干的活。shadow 数据可佐证：
`observed_sleep_seconds == planned_wait` 精确相等，说明睡够了，晚的全是 sleep 之外
的工时，一轮约 2830 步累成 24 分钟。收敛点 ≈ `周期 / 61` ≈ 23.6 分钟，与实测
2 → 16 → 24 分钟的爬升一致。该项已随 #346 修好（每步按 `due - now()` 重算）。

现行口径（`sync_alert_notifier._alert_conditions`）：

- stale 阈值 = `max(freshness_sla_seconds, 调度周期 + SYNC_STALE_GRACE_SECONDS)`，
  宽限默认 7200 秒。等价于「下一次应有运行时间之后还没来才算延迟」。
- 调度周期从 `integration_sync_config` 按 provider 取（wecom/feishu → `doc_sync`，
  chanjet → `chanjet`），读不到退回 86400——宁可晚报，不误报。
- **配 SLA 时不必再关心它是否小于周期**，代码会兜底；但也因此，给日更作业配
  1 小时 SLA 不会让它一小时就报，别指望用 SLA 做更紧的监控。

⚠️ 三条同族坑放在一起看：本节（阈值撞周期）、上一节（告警器与页面是两份独立部署，
判据可以不一致）、`AGENTS.md`〈注册型配置必须断言生效时间〉——都是**误报/漏报与
正常在观测面上长得一样**，只能靠「拿一条真实时间线对账」发现。

## 告警通知在发送层按文档聚合（2026-09-01）

一个作业 = 文档里的一张子表，一个文档几十张表同时延迟就是几十条消息。
2026-09-01 起在**发送层**聚合，库里仍然一个作业一行告警：

- 分组键 = `(收件人, 方向)`，方向只分「告警」（open 与 escalate 合并，升级本质是
  同一批告警的再通知）和「恢复」。一次轮询里同一收件人最多两条消息。
- 消息内二级分组：先按 `alert_kind`（**失败永远排在延迟前面**），再按文档折行，
  每个文档最多点名 3 张表、最多列 5 个文档，其余只报数。
- 单条时保持原格式，带 `?job=<job_key>` 深链；聚合消息只能链到 `/sync/`
  （页面目前只认 `job` 参数，没有 `doc`）。
- **`notify_count`、6 小时升级节流（`SYNC_ALERT_ESCALATION_SECONDS`）、解除判定
  全部不变**，聚合不改状态机。
- 文档名取表级来源行的 `external_sources.document_name`（与页面资产视图同一字段），
  取不到时退回 `display_name` 的「文档 / 表」前半段。
  ⚠️ #306 当初断言告警器完全不碰 `external_sources`，2026-09-01 起放开成
  「只按主键 join、只取名字」；同步状态一律仍从 `sync_*` 三张表读。
- outbox 行与告警状态更新落在**同一个事务**里（`deliver_due`/`resolve_alert` 的
  `defer_commit=True` + `notify_client.enqueue(commit=False)`）。任一分组发不出去就
  整轮回滚，下一轮重来——不留「标了已通知、消息却没写进 outbox」这种静默丢失。

按这套规则回放 09-01 那晚：33 条 → 3 条（两次轮询各一条告警 + 一条恢复）。
跨轮次的合并（把相隔 32 秒的两批并成一条）需要给告警行加「待发」标记，未做。

### 怎么手工触发一次告警来验证（旧办法自 2026-09-01 起失效）

改判据之前，复现 stale 的标准动作是「把某张表的 `freshness_sla_seconds` 调小」。
**兜底改成 `max(SLA, 周期 + 宽限)` 之后这招不管用了**——把 SLA 调成 60 秒，阈值仍然是
93600 秒。照旧法试会得到「怎么都不报」，很容易误判成告警坏了。现在有三条路：

| 想验什么 | 怎么做 | 代价 |
|---|---|---|
| stale 判据本身 | 给 worker 加 `SYNC_STALE_GRACE_SECONDS=1` 并重启容器，再把某张表的 SLA 调小 | 要改 compose env + 重启，验完必须撤回 |
| 聚合（多条折一条） | 往 `sync_job_runs` 插两条**同一文档不同表**、`status='failed'` 的记录，等一轮轮询（30 秒）→ 应只收到 1 条聚合告警；删掉记录 → 告警器自己解除，再收 1 条恢复 | 写生产表，页面运行时间线上会出现假失败记录；`sync_job_steps` 级联删、`sync_job_alerts.run_id` 是 SET NULL |
| 文案与投递链路 | 在容器里直接调 `send_feishu_text(chat_id, build_batch_text(...))` | 会往飞书群真发一条，不写业务表 |

⚠️ **`failed` 类告警不受 SLA 影响**（判据是 `latest_run.status in ('failed','partial')`），
所以第二条是新判据下唯一不用改环境变量就能端到端验证聚合的办法。

⚠️ **截至 2026-09-01 部署完成时，聚合这条路一次都没被真实调用过**——单测和反证覆盖了逻辑，
生产上「首次真实调用」那一栏是空的。用户当时的决定是不主动触发、等自然发生。
下一次这块出问题时，先确认是「新逻辑有 bug」还是「它根本没被走过」。
