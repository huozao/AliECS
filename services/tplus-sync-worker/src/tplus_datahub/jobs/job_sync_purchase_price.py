...
def main() -> int:
    logger = get_logger("tplus_datahub.job_sync_purchase_price", "output/logs/job_sync_purchase_price.log")
    timestamp = now_timestamp()
    try:
        settings = load_settings()
        rows = sync_purchase_price(settings=settings, timestamp=timestamp)
        excel_path = export_purchase_price(rows, settings=settings, timestamp=timestamp)
        logger.info("Purchase price Excel exported: %s", excel_path)
        return 0
