...
## v2.1.14：结构备份表头重排 + 企微自录 + 飞书结构备份

- 结构备份总体字段前置；旧工作表通过“新表复制并核对后替换”的方式安全重排字段，不原地破坏数据。
- 将结构备份文档自身登记到企微 A，只同步四张备份工作表的字段结构，不读取备份记录，避免递归。
- 新增 `飞书-最新结构`：每行一个飞书多维表格应用，保存 app_token、table_id、表名和字段结构，并复用结构变更历史。

## v2.1.13：企微智能表格结构自动备份

- 新增企微 A/B 智能表格结构备份：每个文档一行，保存 docid、工作表编码/名称、规范化字段配置，不保存业务记录。
- 备份文档固定包含 `企微A-最新结构`、`企微B-最新结构`、`结构变更历史`；最新结构按文档唯一键更新，历史仅在结构哈希变化时追加。
- 新增持久化备份任务队列、每日自动同步、“创建副本”首次同步后自动入队、指数退避重试及初始化/手工同步 CLI。
