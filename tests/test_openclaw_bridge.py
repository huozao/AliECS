...
def test_bridge_detects_real_feishu_group_with_gmt_timestamp_prefix():
    bridge = load_bridge()

    text = (
        "[Mon 2026-07-27 21:58 GMT+8] Conversation info (untrusted metadata):\n"
        "```json\n"
        "{\n"
        '  "chat_id": "chat:oc_b39807445ba47156b05666ce457e78bf",\n'
        '  "message_id": "om_x100b6942716de0acb299e425c37c57e",\n'
        '  "sender_id": "ou_28d4f058cbd2a13f3fcc6fd575023e8e",\n'
        '  "conversation_label": "HAO",\n'
        '  "sender": "hao",\n'
        '  "timestamp": "Mon 2026-07-27 21:58:18 GMT+8",\n'
        '  "group_subject": "HAO",\n'
        ...
        '"id":"ou_28d4f058cbd2a13f3fcc6fd575023e8e","name":"hao"}\n'
        "```\n\n"
        "hao: /新对话 帮我看看吉隆坡明天天气"
    )
    ...
    assert outbound["metadata"]["chat_type"] == "group"
    assert outbound["metadata"]["peer_id"] == "group:oc_b39807445ba47156b05666ce457e78bf"
    assert outbound["metadata"]["message_id"] == "om_x100b6942716de0acb299e425c37c57e"
    assert outbound["messages"][0]["content"] == "/新对话 帮我看看吉隆坡明天天气"
...
def test_deliver_feishu_text_card_sends_card_with_footer(monkeypatch):
    ...
    assert [e["tag"] for e in els] == ["markdown", "markdown"]
    assert els[0]["content"] == "纯文字回复"
    assert "设备: webdock2(主) | 耗时: 9s" in els[1]["content"]
...
def test_deliver_feishu_media_sends_ordered_card_and_returns_no_reply(monkeypatch):
    ...
    assert mid == "om_1"
    els = card["body"]["elements"]
    assert [e["tag"] for e in els] == ["markdown", "img", "markdown", "img"]
    assert els[1]["img_key"] == "key:http://h/media/img1"
    assert els[3]["img_key"] == "key:http://h/media/img2"
...
def test_build_feishu_card_converts_headings_to_bold():
    ...
    # JSON 2.0 markdown accepts standard ATX headings without the 1.0 rewrite.
...
def test_build_feishu_card_appends_footer_note():
    ...
    assert "设备: webdock1(主) | 耗时: 12s" in els[2]["content"]
...
def test_build_feishu_card_marks_update_multi():
    ...
    # update_multi 是 feishu_patch_card 原地更新占位卡的前提，2.0 不能丢
    assert card["config"]["update_multi"] is True
    assert card["schema"] == "2.0"
    assert card["config"]["width_mode"] == "fill"
    assert "body" in card
    dumped = json.dumps(card, ensure_ascii=False)
    for removed in ('"tag": "note"', '"tag": "action"', '"tag": "lark_md"', '"wide_screen_mode"'):
...
def test_colleague_mention_is_not_forwarded_in_mention_only_groups(monkeypatch):
    ...
    assert bridge.feishu_should_send_chatgpt(details) is False
    fields = bridge.build_feishu_message_log_fields(details, reply="", status="未处理")
    assert fields["是否 @ 机器人"] is False
    assert fields["不处理原因"] == "未@机器人"
