...
def overview_row(
    *,
    ...
    return (
        ...
        error_kind,
        "sanitized failure" if error_kind else None,
        (detail_json if detail_json is not None else {"phase": "done"}) if has_run else None,
        {"table": "sync_runs", "id": 7} if has_run else None,
        last_success_at,
        # 新鲜度看的是「最近一次确认」（成功或整簿跳过），默认与成功时刻同值。
        last_success_at if last_verified_at is None else last_verified_at,
        open_alert_count,
        env_profile,
        document_name,
        sheet_name,
